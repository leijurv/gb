...
func DirHistory(basePath string) {
	...
	rows, err := db.DB.Query(`SELECT path, COUNT(*) AS num_revisions, MIN(start) AS first_backup, MAX(fs_modified) AS max_fs_modified, MIN(COALESCE(end, 0)) AS min_end FROM files WHERE path GLOB ? AND path NOT GLOB ? GROUP BY path`, basePath+"*", basePath+"*/*/*")
	if err != nil {
		panic(err)
	}
	defer rows.Close()
